...
export async function POST(req: NextRequest) {
  ...
  const topics: string[] = [body.topic, ...(Array.isArray(body.extraTopics) ? body.extraTopics : [])].filter(
    Boolean
  );
  ...
  const scored: ScoredCandidate[] = classified
    .filter((entry) => entry.classification && entry.classification.isLeadUserSignal)
    .map((entry) => ({ ...entry.candidate, ...(entry.classification as NonNullable<typeof entry.classification>) }))
    .sort((a, b) => b.solubilityScore + b.expectedBenefitScore - (a.solubilityScore + a.expectedBenefitScore));
  ...
  return NextResponse.json<DiscoverResponse>({ candidates: scored, themes, sourceNotes });
}
...
async function fetchHackerNews(topics: string[]): Promise<Candidate[]> {
  const results = await Promise.all(topics.map((t) => searchHackerNews(t, HN_LIMIT)));
  return results.flat();
}
